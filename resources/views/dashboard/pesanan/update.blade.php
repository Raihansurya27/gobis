...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Update Pesanan Tiket Bus</h1>
</div>
    <div class="row">
        <div class="col-lg-6">
            <form action="{{url('/pesanan/'.$pesanan->id)}}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control" type="text" name="nama" id="nama" disabled="true" value="{{$pesanan->user->nama}}">
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select" aria-label="Default select example" name="jadwal_id" id="jadwal_id">
                        @forelse ($jadwals as $jadwal)
                            @if (old('jadwal_id',$pesanan->jadwal_id) == $jadwal->id)
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" aria-label="Default select example" name="status" id="status">
                        <option value="dipesan" @if (old('status',$pesanan->status) == "dipesan")
                            selected
                        @else

                        @endif>Dipesan</option>
                        <option value="dibayar" @if (old('status',$pesanan->status) == "dibayar")
                            selected
                        @else
                ...
                <div class="mb-3">
                    ...
                    @enderror"
                        id="tanggal_pesan" name="tanggal_pesan" value="{{ ucwords(old('tanggal_pesan',$pesanan->tanggal_pesan)) }}">
                    @error('tanggal_pesan')
                        {{ $message }}
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    @enderror"
                        id="tanggal_beli" name="tanggal_beli" value="{{ ucwords(old('tanggal_beli',$pesanan->tanggal_beli)) }}">
                    @error('tanggal_beli')
                        {{ $message }}
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    @enderror"
                        id="jumlah" placeholder="Contoh: 3" name="jumlah" value="{{ ucwords(old('jumlah',$pesanan->jumlah)) }}">
                    @error('jumlah')
                        {{ $message }}
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    @enderror"
                        ...
                        value="{{ ucwords(old('total')) }}" readonly="true" onclick="totalFun()" value="{{old('total',$pesanan->total)}}">
                    @error('total')
                        {{ $message }}
                    @enderror
                </div>
