        ...
        @forelse ($users as $user)
            <tr>
                ...
                <td id="pendek-{{$loop->iteration}}">
                    {{ Str::limit(implode('', [$user->alamat, $user->kelurahan->nama, $user->kelurahan->kecamatan->nama, $user->kelurahan->kecamatan->kabupaten->nama, $user->kelurahan->kecamatan->kabupaten->provinsi->nama]), 5) }}<button
                        onclick="tampilkan({{ $loop->iteration }})"><span data-feather="arrow-down-right"></span>
                    </button>
                </td>
                <td id="panjang-{{$loop->iteration}}" hidden="true">{{ $user->alamat }}, {{ $user->kelurahan->nama }},
                    {{ $user->kelurahan->kecamatan->nama }}, {{ $user->kelurahan->kecamatan->kabupaten->nama }},
                    {{ $user->kelurahan->kecamatan->kabupaten->provinsi->nama }} <button onclick="sembunyikan({{ $loop->iteration }})"><span
                            data-feather="arrow-up-left"></span></button></td>
                <td>
