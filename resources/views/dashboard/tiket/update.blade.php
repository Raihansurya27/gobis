...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Update Data Tiket Bus</h1>
    </div>
    <div class="row">
        <div class="col-lg-6">
            <form action="{{ url('/tiket/' . $tiket->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    @enderror"
                        id="kode_tiket" placeholder="Contoh: BKT002-01-02" name="kode_tiket"
                        value="{{ ucwords(old('kode_tiket', $tiket->kode_tiket)) }}">
                    @error('kode_tiket')
                        {{ $message }}
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" aria-label="Default select example" name="pesanan_id" id="pesanan_id"
                        ...
                        @forelse ($pesanans as $pesanan)
                            @if (old('pesanan_id', $tiket->pesanan_id) == $pesanan->id)
                                <option value="{{ $pesanan->id }}" selected>
                                    {{ ucwords('' . $pesanan->jadwal->rute->awal->nama . '->' . $pesanan->jadwal->rute->tujuan->nama . ', Keberangkatan : ' . $pesanan->jadwal->keberangkatan) }}
                                </option>
                            @else
